        ...
        if len(request.FILES) != 0:
            ...
            if name.endswith('.jpg') or name.endswith('.png') or name.endswith('.jpeg'):
                ...
                if form.is_valid():
                    ...
                    filename = number_plate_image.img.file

                    output_numbers = main_annpr_detector(
                        detector='image', filename=filename.name)
...
def upload_video(request):
    if request.method == "POST":
        ...
        file = request.FILES.get('video').name
        print(f'Filename: {file}')
        if file.endswith('.mp4') or file.endswith('.avi'):
            print(f' {file} File is a video')
            if form.is_valid():
                number_plate_video = form.save(commit=False)
                number_plate_video.name = file
                number_plate_video.save()
                filename = number_plate_video.video.file
                output_numbers = main_annpr_detector(detector='video', filename=filename.name)
